...
class GenericProgressBar extends Sprite
{
    private static inline var WIDTH : Float = 200.0;
    private static inline var HEIGHT : Float = 20.0;
    ...
    public function new(_task : String = "")
    {
        super();
    ...
    /**
		 ...
		 */
    public function update(pct : Float) : Void
    {
        pct = Math.max(Math.min(pct, 1.0), 0.0);
        progress_text.text = " " + Std.string(Math.round(pct * 100)) + "%";
        progress_text.setTextFormat(format);
        graphics.clear();
