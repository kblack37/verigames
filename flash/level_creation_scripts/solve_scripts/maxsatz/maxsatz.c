...
/* the tables of variables and clauses are statically allocated. Modify the 
   ...
   necessary */
#define tab_variable_size  500000
#define tab_clause_size 1500000
...
int build_simple_sat_instance(char *input_file) {
  ...
  printf("c Instance info: p %s %d %d %d %d %lli\n", 
	 word2, NB_VAR, NB_CLAUSE, HARD_WEIGHT, tab_variable_size, INIT_BASE_NB_CLAUSE);
...
void lire_clauses_string(char *ch) {
  int i, j, jj, ii, length, tautologie, lits[1000], lit, lit1, offset, weight;

  for (i=0; i<NB_CLAUSE; i++) {
    length=0; 
    sscanf(ch, "%d%n", &weight, &offset);
	ch += offset;
	 sscanf(ch, "%d%n", &lits[length], &offset);
	  ch += offset;
    while (lits[length] != 0) {
      length++;
     sscanf(ch, "%d%n", &lits[length], &offset);
...
my_type build_simple_sat_instance_string(char *input_string) {

  char *ch, word2[WORD_LENGTH], *ctemp;
  int i, j, length, NB_CLAUSE1, res, ii, jj, tautologie, lit1,
    lits[1000], *lits1, lit, var, *pos_nb, *neg_nb;
  int clause,*vars_signs,cpt, foo;
  ...
  ch += 7;
  sscanf(ch, "%d%n", &NB_VAR,  &offset);
  ch += offset;
  sscanf(ch, "%d%n",  &NB_CLAUSE, &offset);
  ch += offset;
  INIT_NB_CLAUSE = NB_CLAUSE;
 printf("%d %d", NB_VAR, NB_CLAUSE);
  lire_clauses_string(ch);
  build_structure();
  eliminate_redundance();
  if (clean_structure()==FALSE)
    return FALSE;
...
void printSolution(char* solpath, int ub)
{
	char solutionName[100];
	int i;
	sprintf(solutionName, "%sub%d.txt", solpath, ub);
	FILE* outputFile = fopen(solutionName, "w");
	for (i = 0; i < NB_VAR; i++) {
...
int main(int argc, char *argv[]) {
  ...
  if (argc <= 1) {
    printf("Using format: %s [-s] input_instance [-i dirpath]\n\t-s: treat input instance is string, else as a file.\n\t-i: output intermediate results to directory.", argv[0]);
    return FALSE;
  }
  ...
  //might have a string arg
  if(argc < 3 || (strcmp(argv[1], "-s") != 0))
  {
	  result = build_simple_sat_instance(argv[1]);
  }
  else if(strcmp(argv[1], "-s") != 0)
  {
	  result = build_simple_sat_instance(argv[2]);
  }
  //might have an intermediate output arg
