...
public class DatabaseConfig {
  ...
  @SuppressWarnings("trusted") // only trusted things can be put.
  public @Trusted String get(String key) {
    return config.get(key);
  }

  public void put(@Trusted String key, @Trusted String value) {
  ...
  public HashMap<@Trusted String, @Trusted String> startWith(String key) {
    HashMap<@Trusted String, @Trusted String> transformer = new HashMap<@Trusted String, @Trusted String>();
    Iterator<?> entries = config.iterator();
    while (entries.hasNext()) {
      String entry = entries.next().toString();
      if (entry.startsWith(key)) {
        @Trusted String[] metrics = (@Trusted String[]) entry.split("=");
  ...
  @SuppressWarnings("trusted") // these longs are trusted. would take lots of annotations. 
  public @Trusted String[] findTableName(/*@Regex*/ @Trusted String tableName, long start, long end) {
    @Trusted String[] tableNames = null;
    @Trusted String tableType = "_week";
    ...
    if (startPartition != endPartition) {
      ...
      for (int i = 0; i <= delta; i++) {
        ...
        tableNames[i] = tableName + "_" + (@Trusted long) partition + tableType;
      }
    } else {
      tableNames = new String[1];
      tableNames[0] = tableName + "_" + (@Trusted long) startPartition + tableType;
    }
    return tableNames;
  }

  public @Trusted String[] findTableNameForCharts(@Trusted String tableName, long start, long end) {
