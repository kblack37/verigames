...
/**
	 ...
	 * @author Rich
	 */
class StripCompareTargetAnimator implements IFractionAnimator
{
    ...
    {
        return CgsFVConstants.STRIP_STANDARD_COMPARE_BENCHMARK;
    }
...
//        var offsetY : Float = firstModule.unitHeight / 2 + StripConstants.TICK_EXTENSION_DISTANCE * 2 + StripConstants.ANIMATION_MARGIN_VERTICAL_SMALL * 2;
...
//        benchmarkNR.x = ((!isFirstLeftAligned && !isSecondLeftAligned)) ? benchmarkX + benchmarkNR.width / 2 + StripConstants.ANIMATION_MARGIN_HORIZONTAL_SMALL : benchmarkX - benchmarkNR.width / 2 - StripConstants.ANIMATION_MARGIN_HORIZONTAL_SMALL;
//        benchmarkNR.y = 0;
//
//        // Benchmark Line
...
//            firstCompareSprite.alpha = StripConstants.FILL_ALPHA_COLORED;
...
//            var firstComparisonSpriteMaskHeight : Float = firstCompareSprite.height + 6;
//            var firstComparisonSpriteMaskWidth : Float = firstCompareSprite.width + 6;
//            var firstComparisonSpriteMask : Sprite = m_animHelper.createMask(firstCompareSprite, firstComparisonSpriteMaskWidth, firstComparisonSpriteMaskHeight);
...
//            secondCompareSprite.alpha = StripConstants.FILL_ALPHA_COLORED;
...
//            var secondComparisonSpriteMaskHeight : Float = secondCompareSprite.height + 6;
//            var secondComparisonSpriteMaskWidth : Float = secondCompareSprite.width + 6;
//            var secondComparisonSpriteMask : Sprite = m_animHelper.createMask(secondCompareSprite, secondComparisonSpriteMaskWidth, secondComparisonSpriteMaskHeight);
...
//        var winningModule : StripFractionModule;
...
//            winningModule = try cast(winningView.module, StripFractionModule) catch(e:Dynamic) null;
//            winningGlowFinalPosition = ((winnerValue > 0)) ? oldFirstPosition : oldSecondPosition;
//
//            // Glow
//            winningGlowLine = new Sprite();
//            var winningWidth : Float = winningModule.totalWidth + StripConstants.WINNING_GLOW_THICKNESS;
//            var winningHeight : Float = winningModule.unitHeight + StripConstants.WINNING_GLOW_THICKNESS;
...
//        benchmarkText.y = newFirstPosition.y - firstModule.unitHeight / 2 - StripConstants.TICK_EXTENSION_DISTANCE - StripConstants.ANIMATION_MARGIN_TEXT - benchmarkText.height / 2;
...
//
//        // Tween into position
//        var position_unitDelay : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DELAY_AFTER_POSITION);
//        var position_unitDuration : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DURATION_POSITION);
...
//        var showBenchmark_unitDelay : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DELAY_AFTER_SHOW_BENCHMARK);
//        var showBenchmark_unitDuration : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DURATION_SHOW_BENCHMARK);
...
//                    alpha : 1
//                }));
//        showBenchmarkStep.addTweenSet(showBenchmarkValueStart, FVEmphasis.computePulseTweens(benchmarkNR, showBenchmark_unitDuration, 1.5, 1.5, StripConstants.PULSE_SCALE_GENERAL));
//        m_animHelper.appendStep(showBenchmarkStep);
//
//        // Compare
//        if (doCompare)
//        {
//            var compare_unitDelay : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DELAY_AFTER_COMPARE);
//            var compare_unitDuration : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DURATION_COMPARE);
...
//            var showFirstCompareLine_startTime : Float = hideFirstSegment_startTime + compare_unitDuration / 2;
//            var moveFirstCompareLine_startTime : Float = showFirstCompareLine_startTime + compare_unitDuration + compare_unitDelay;
//            var showFirstSegment_startTime : Float = moveFirstCompareLine_startTime + compare_unitDuration;
//            var hideSecondSegment_startTime : Float = showFirstSegment_startTime + compare_unitDuration / 2 + compare_unitDelay;
//            var showSecondCompareLine_startTime : Float = hideSecondSegment_startTime + compare_unitDuration / 2;
//            var moveSecondCompareLine_startTime : Float = showSecondCompareLine_startTime + compare_unitDuration + compare_unitDelay;
//            var showSecondSegment_startTime : Float = moveSecondCompareLine_startTime + compare_unitDuration;
...
//                        alpha : StripConstants.FILL_ALPHA_COLORED,
...
//        var showResult_unitDelay : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DELAY_AFTER_SHOW_RESULT);
//        var showResult_unitDuration : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DURATION_SHOW_RESULT);
...
//                        ease : Sine.easeInOut
//                    }));
//            showResultStep.addTweenSet(showResult_emphasizeWinner_startTime, FVEmphasis.computePulseTweens(winningGlowLine, showResult_unitDuration / 2, 1, 1, StripConstants.PULSE_SCALE_GENERAL));
//            showResultStep.addTweenSet(showResult_winnerPulse_startTime, FVEmphasis.computePulseTweens(winningNR, showResult_unitDuration, 1, 1, StripConstants.PULSE_SCALE_GENERAL_LARGE));
...
//                        ease : Sine.easeInOut
//                    }));
//            showResultStep.addTweenSet(.1, FVEmphasis.computePulseTweens(equalTextHolder, showResult_unitDuration, 1, 1, StripConstants.PULSE_SCALE_GENERAL));
//        }
//        m_animHelper.appendStep(showResultStep);
//
//        // Fade out
//        var fadeOut_unitDelay : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DELAY_AFTER_FADE);
//        var fadeOut_unitDuration : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DURATION_FADE);
...
//
//        // Final Position
//        var unposition_unitDelay : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DELAY_AFTER_UNPOSITION);
//        var unposition_unitDuration : Float = m_animHelper.computeScaledTime(StripConstants.TIME_COMPARE_TARGET_DURATION_UNPOSITION);
