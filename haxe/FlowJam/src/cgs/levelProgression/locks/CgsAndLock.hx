...
/**
	 * This lock contains a list of other locks and is unlocked if all of its locks are unlocked
	 * @author Jack
	 */
class CgsAndLock implements ICgsLevelLock
{
    ...
    //Lock Type Key
    public static inline var LOCK_TYPE : String = "andLock";
    ...
    {
        while (m_locks.length > 0)
        {
            lockFactory.recycleLock(m_locks.pop());
        }
    }
    ...
    private function checkLock() : Bool
    {
        ...
        if (m_locks != null && m_locks.length > 0)
        {
            ...
            {
                var lockIsLocked : Bool = aLock.isLocked;
                result = result || lockIsLocked;
                // If we find a locked lock, we break, because we only need one locked lock to be locked
                if (lockIsLocked)
                {
                    break;
                }
