...
class Geometry
{
    private static var m_locked : Bool = false;
    ...
    private static var half : Float = 0.5;
    private static var dx : Float;  // x-diff between endpoints  
    private static var dy : Float;  // y-diff between endpoints  
    private static var m : Float;  // diff between endpoints  
    private static var b : Float;  // diff between endpoints  
    private static var sq_dist1 : Float;  // dist squared value from control point 1 to line between endpoints  
    private static var sq_dist2 : Float;  // dist squared value from control point 2 to line between endpoints  
    /**
		 ...
		 */
    public static function splineToLineSegments(p1x : Float, p1y : Float, c1x : Float, c1y : Float, c2x : Float, c2y : Float, p2x : Float, p2y : Float, tolerance : Float = 1.5, prev_lines : Array<Dynamic> = null) : Array<Dynamic>
    {
        if (m_locked)
        {
        ...
        //dev note:these cannot be static external variables for performance gain, as they are required to maintain previous values on return from recusive execution{
            ...
            var p01x : Float = (p1x + c1x) * half;
            var p01y : Float = (p1y + c1y) * half;
            var p12x : Float = (c1x + c2x) * half;
            var p12y : Float = (c1y + c2y) * half;
            var p23x : Float = (c2x + p2x) * half;
            var p23y : Float = (c2y + p2y) * half;
            var p02x : Float = (p01x + p12x) * half;
            var p02y : Float = (p01y + p12y) * half;
            var p13x : Float = (p12x + p23x) * half;
            var p13y : Float = (p12y + p23y) * half;
            var p03x : Float = (p02x + p13x) * half;
            var p03y : Float = (p02y + p13y) * half;
    ...
    private static var p1_weight : Float;
    private static var p2_weight : Float;
    private static var p3_weight : Float;
    private static var p4_weight : Float;
    /**
		 ...
		 */
    public static function getXYbyTSpline(spline_control_points : Array<Dynamic>, input_t : Float) : Array<Dynamic>
    {
        if (m_locked)
        {
            throw new Error("Call to Geometry.getXYbyTSpline when class is locked - check for concurrent calls");
            return new Array<Dynamic>(0, 0);
        }
        ...
        m_locked = true;
        var mtx_a : Array<Dynamic> = new Array<Dynamic>(4);
        var mtx_b : Array<Dynamic> = new Array<Dynamic>(4);
        ...
        m_locked = false;
        return new Array<Dynamic>(p1_weight * p1.x + p2_weight * p2.x + p3_weight * p3.x + p4_weight * p4.x, p1_weight * p1.y + p2_weight * p2.y + p3_weight * p3.y + p4_weight * p4.y);
    }
}
