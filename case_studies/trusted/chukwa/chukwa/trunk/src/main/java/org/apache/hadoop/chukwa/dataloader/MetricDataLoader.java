...
public class MetricDataLoader implements Callable<Boolean> {
  private static Log log = LogFactory.getLog(MetricDataLoader.class);

  private Statement stmt = null;
  private ResultSet rs = null;
  private DatabaseConfig mdlConfig = null;
  private HashMap<@Trusted String, @Trusted String> normalize = null;
  private HashMap<@Trusted String, @Trusted String> transformer = null;
  private HashMap<String, Float> conversion = null;
  private HashMap<@Trusted String, @Trusted String> dbTables = null;
  ...
  private void initEnv(String cluster) throws Exception {
    ...
    HashMap<@Trusted String, @Trusted String> dbNames = mdlConfig.startWith("report.db.name.");
    ...
    while (ki.hasNext()) {
      ...
      try {
        ...
        while(rs.next()) {
          @SuppressWarnings("trusted") // library call, comes from database
          @Trusted String name = (@Trusted String) rs.getString("COLUMN_NAME");
          ...
          @SuppressWarnings("trusted") // created in stringBuilder from all trusted strings
          @Trusted String mdlKey = (@Trusted String) metricName.toString().toLowerCase();
    ...
    try {
      ...
      while (reader.next(key, record)) {
        ...
        if (dbTables.containsKey(dbKey)) {
          ...
          String[] tmp = mdlConfig.findTableName(tableName, record.getTime(),
              record.getTime());
          table = tmp[0];
        } else if(m.matches()) {
