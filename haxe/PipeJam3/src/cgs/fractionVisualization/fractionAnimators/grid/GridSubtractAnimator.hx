...
/**
	 ...
	 * @author Rich
	 */
class GridSubtractAnimator implements IFractionAnimator
{
    ...
    {
        return CgsFVConstants.GRID_STANDARD_SUBTRACT;
    }
...
//        result.fraction.init(finalFirstFraction.numerator, finalFirstFraction.denominator);
...
//        }
//        // Set result fraction now, after we get the drop destinations, so that we have a destination for each block of the first
...
//
//            // Get its drop destination
//            var indexFromFirst : Int = as3hx.Compat.parseInt(dropSegmentDestinationsOfFirst.length - finalSecondFraction.numerator + dropSegmentIndex);
//            aDropDestination = dropSegmentDestinationsOfFirst[indexFromFirst];
...
//			**/
//
//        var position_unitDelay : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DELAY_AFTER_POSITION);
//        var position_unitDuration : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DURATION_POSITION);
...
//        if (doChangeDenom)
//        {
//            var changeDenom_unitDelay : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DELAY_AFTER_CHANGE_DENOM);
//            var changeDenom_unitDuration : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DURATION_CHANGE_DENOM);
...
//
//        // Drop
//        var drop_unitDelay : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DELAY_AFTER_DROP);
//        var drop_unitDuration : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DURATION_DROP);
//        var drop_durationPerBlock : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DURATION_DROP_PER_BLOCK);
...
//        var aDropMoveDuration : Float = (totalDropDuration / (dropSegmentsOfSecond.length + dropSegmentsOfFirst.length + 1)) / nextBlockDelayModifier;  // Each block starts moving after the last block has gotten half way
//        var dropFirstValue_startTime : Float = startDropMoveTime + totalDropDuration + drop_unitDuration;
...
//        }
//        dropSegmentIndex = dropSegmentsOfSecond.length - 1;
//        while (dropSegmentIndex >= 0)
//        {
//            // Add the blocks to the display list
...
//                        y : aDropDestination.y
//                    }));
//            var shrinkStartTime : Float = thisDropMoveTime + aDropMoveDuration;
//            thisDropMoveTime += aDropMoveDuration * nextBlockDelayModifier;
//
//            // Handle remove segments
//            var removeSegmentIndex : Int = as3hx.Compat.parseInt(dropSegmentIndex + dropSegmentsOfFirst.length - dropSegmentsOfSecond.length);  // Index of the segment to be removed from the first
//            var aRemoveSegment : Sprite = dropSegmentsOfFirst[removeSegmentIndex];
//            dropStep.addTween(shrinkStartTime, new GTween(aRemoveSegment, drop_unitDuration / 4, {
//                        alpha : 0
//                    }));
//
//            // Shrink
//            dropStep.addTweenSet(shrinkStartTime, FVEmphasis.computeNullifyTweens(aDropSegment, drop_unitDuration, 1, 1, GridConstants.PULSE_SCALE_SEGMENT));
//            dropSegmentIndex--;
//        }
...
//
//        // Tween Merge
//        var merge_unitDelay : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DELAY_AFTER_MERGE);
//        var merge_unitDuration : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DURATION_MERGE);
...
//        if (doSimplify)
//        {
//            var simplification_unitDelay : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DELAY_AFTER_SIMPLIFICATION);
//            var simplification_unitDuration : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DURATION_SIMPLIFICATION);
...
//
//        // Final Position
//        var unposition_unitDelay : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DELAY_AFTER_UNPOSITION);
//        var unposition_unitDuration : Float = m_animHelper.computeScaledTime(GridConstants.TIME_SUB_DURATION_UNPOSITION);
...
//        }
//
//        function finalizeChangeDenomOfSecond_reverse() : Void
//        {
//            // Update second fraction
...
//                aTickSprite.visible = true;
//            }
//        }  /*function prepForDrop():void
//			{
//				// Setup drop segments and remvoe segments
//				var dropSegmentTuple:CgsTuple = secondModule.peelBlocks(dropSegments);
//				var dropSegmentLocations:Vector.<Point> = dropSegmentTuple.second;
//				var removeSegmentTuple:CgsTuple = firstModule.peelBlocks(removeSegments, exclusionList);
//				var removeSegmentLocations:Vector.<Point> = removeSegmentTuple.second;
//				for (dropSegmentIndex = 0; dropSegmentIndex < finalSecondFraction.numerator; dropSegmentIndex++)
//				{
//					// Show the drop segments
//					aDropSegment = dropSegments[dropSegmentIndex];
...
//
//					// Show the remove segments
//					aRemoveSegment = removeSegments[dropSegmentIndex];
//					aRemoveSegment.visible = true;
//
//					// Adjust their locations
//					var aRemoveLocation:Point = removeSegmentLocations[dropSegmentIndex];
//					aRemoveSegment.x = first.x + aRemoveLocation.x;
//					aRemoveSegment.y = first.y + aRemoveLocation.y;
//				}
//
//				// Hide segments
//				secondModule.doShowSegment = false;
...
//				// Show result
//				result.visible = true;
//				result.x = newFirstPosition.x - firstModule.totalWidth / 2 + resultModule.totalWidth / 2;
...
//			{
//				// Hide drop segments
//				for each (aDropSegment in dropSegments)
//				{
//					aDropSegment.visible = false;
//				}
//
//				// Hide remove segments
//				for each (aRemoveSegment in removeSegments)
//				{
//					aRemoveSegment.visible = false;
//				}
//
//				// Show segments
//				secondModule.doShowSegment = true;
...
//			}
//
//			function finishDrop_reverse():void
//			{
//				// Hide result value
//				resultModule.valueNumDisplayAlpha = 0;
//				result.redraw(true);
...
//
//			// Redraw the first as the result fraction
//			function finalizeResult_reverse():void
//			{
//				// Show fractions
//				second.visible = true;
