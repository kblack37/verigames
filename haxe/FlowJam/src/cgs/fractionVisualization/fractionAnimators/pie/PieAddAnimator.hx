...
/**
	 ...
	 * @author Mike
	 */
class PieAddAnimator implements IFractionAnimator
{
    ...
    {
        return CgsFVConstants.PIE_STANDARD_ADD;
    }
...
//        var addSymbolText : TextField = showEqData[1];
...
//        firstValueNR.x = 0;
//        firstValueNR.y = 0;
//        var firstValueNRFinalPosition : Point = new Point(equationCenterX - secondValueNR.width / 2 - addSymbolText.width - firstValueNR.width / 2, equationCenterY);
//        addSymbolText.x = equationCenterX - secondValueNR.width / 2 - addSymbolText.width;
//        addSymbolText.y = equationCenterY - addSymbolText.height / 2;
//        secondValueNR.x = 0;
//        secondValueNR.y = 0;
...
//
//        // Tween into position
//        var position_unitDelay : Float = m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DELAY_AFTER_POSITION);
//        var position_unitDuration : Float = m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DURATION_POSITION);
//
//
//        var firstValueNRPosition : Point;
//        var firstDenom_equationPosition : Point;
//        var firstDenom_eqData : EquationData;
//
//
//        // Get equation data for changing the second denomintor
//        var secondDenom_equationPosition : Point;
//        var secondDenom_eqData : EquationData;
//
//
//
//
...
//        if (doChangeDenom)
//        {
//            var changeDenom_unitDelay : Float = m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DELAY_AFTER_CHANGE_DENOM);
//
//            var changeDenom_unitDuration : Float = m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DURATION_CHANGE_DENOM);
//
//            /**
//				 * Consolidate first fraction
//				**/
//            // Change First Denominator Equation Data
//            firstValueNRPosition  = firstModule.getValueNRPosition(true, first.fraction.denominator == 1);
//            firstDenom_equationPosition  = new Point(newFirstPosition.x + firstValueNRPosition.x, newFirstPosition.y + firstValueNRPosition.y);
//            firstDenom_eqData  = m_animHelper.createEquationData(m_animController, firstDenom_equationPosition, first.fraction, " × ", firstMultiplierFraction, finalFirstFraction, textColor, textGlowColor);
...
//
//            // Get equation data for changing the second denomintor
//            secondDenom_equationPosition  = new Point(newSecondPosition.x + secondModule.valueNRPosition.x, newSecondPosition.y + secondModule.valueNRPosition.y);
//            secondDenom_eqData  = m_animHelper.createEquationData(m_animController, secondDenom_equationPosition, second.fraction, " × ", secondMultiplierFraction, finalSecondFraction, textColor, textGlowColor);
...
//
//        // Parts to drop
//        var firstFullCircles : Array<Sprite> = firstModule.duplicateFullCircles(firstScale);
//        var secondFullCircles : Array<Sprite> = secondModule.duplicateFullCircles(secondScale);
//        var firstSectors : Array<Sprite> = firstModule.duplicatePartialSprites(firstScale);
//        var secondSectors : Array<Sprite> = secondModule.duplicatePartialSprites(secondScale);
//
//
//
//        // Tween drop
//        var drop_unitDelay : Float = m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DELAY_AFTER_DROP);
//        var drop_unitDuration : Float = m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DURATION_DROP);
//
...
//        for (i in 0...firstFullCircles.length)
//        {
//            m_animController.addChild(firstFullCircles[i]);
//            m_animHelper.trackDisplay(firstFullCircles[i]);
//            var moveFirstCircles : GTween = new GTween(firstFullCircles[i], drop_unitDuration, {
...
//        }
//        // if there are some circles, add in the drop time above
//        ((runningDropTime != 0 && !Math.isNaN(runningDropTime)) += (firstFullCircles.length)) ? drop_unitDuration : 0;
//
...
//        }
//        // if there are some circles, add in the drop time above
//        ((runningDropTime != 0 && !Math.isNaN(runningDropTime)) += (secondFullCircles.length)) ? drop_unitDuration : 0;
//
...
//            if (i == 0)
//            {
//                m_animController.addChild(firstSectors[i]);
//                indexOfFirstSector = m_animController.getChildIndex(firstSectors[i]);
//            }
//            else
//            {
//                m_animController.addChildAt(firstSectors[i], indexOfFirstSector);
//            }
//            m_animHelper.trackDisplay(firstSectors[i]);
//            var movefirstSectors : GTween = new GTween(firstSectors[i], durationOfFirstSectors, {
//                x : runningCenterValue,
//                y : result.y,
//                alpha : 1.0
//            }, {
//                ease : Sine.easeInOut
//            });
//            dropStep.addTween(runningDropTime, movefirstSectors);
//            runningDropTime += timingOfFirstSectors;
//        }
//        // adjust for the sector that is still moving
//        ((runningDropTime != 0 && !Math.isNaN(runningDropTime)) += (firstSectors.length)) ? durationOfFirstSectors : 0;
//
//        var timingOfSecondSector : Float = drop_unitDuration / 4;
//        var durationOfSecondSector : Float = timingOfSecondSector * 2;
//
//        // rotate last set back to meet end of previous partial circle
//        var secondSectorRotation : Float = (Math.ceil(first.fraction.value) - first.fraction.value) * 360;
//
...
//                indexOfSecondSector = m_animController.getChildIndex(secondSectors[i]);
//            }
//            else
//            {
//                m_animController.addChildAt(secondSectors[i], indexOfSecondSector);
//            }
//            m_animHelper.trackDisplay(secondSectors[i]);
//            var moveToNextCircle : Bool = (i * (1 / commonDenominator) + first.fraction.value) == Math.ceil(first.fraction.value);
//            if (moveToNextCircle)
//            {
//                runningCenterValue += spanBetweenPieCenters;
//            }
//
//            var moveSectors : GTween = new GTween(secondSectors[i], durationOfSecondSector, {
//                x : runningCenterValue,
//                y : result.y,
//                alpha : 1.0,
...
//            runningDropTime += timingOfSecondSector;
//        }
//        // adjust for the sector that is still moving
//        ((runningDropTime != 0 && !Math.isNaN(runningDropTime)) += (secondSectors.length)) ? durationOfSecondSector : 0;
//
...
//
//        // Tween Merge
//        var merge_unitDelay : Float = m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DELAY_AFTER_MERGE);
//        var merge_unitDuration : Float = m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DURATION_MERGE);
...
//        var consolidateEquationStart : Float = fadeInResultValueStart + merge_unitDuration + merge_unitDelay;  // Includes a little delay after the result text is visible
...
//        for (i in 0...firstFullCircles.length)
//        {
//            var fadeFirstCircles : GTween = new GTween(firstFullCircles[i], merge_unitDuration, {
//                alpha : 0.0
//            });
...
//        for (i in 0...firstSectors.length)
//        {
//            var fadefirstSectors : GTween = new GTween(firstSectors[i], merge_unitDuration, {
//                alpha : 0.0
//            });
//            mergeStep.addTween(fadeInResultSectorsStart, fadefirstSectors);
//        }
//
//        for (i in 0...secondSectors.length)
//        {
...
//        }
//
//        mergeStep.addTweenSet(fadeInResultValueStart, EquationData.animationEquationInline_phaseTwo(eqData, merge_unitDuration * (3 / 4)));
...
//
//        // Final Position
//        var unposition_unitDelay : Float = m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DELAY_AFTER_UNPOSITION);
//        var unposition_unitDuration : Float = m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DURATION_UNPOSITION);
//
...
//            for (i in 0...firstFullCircles.length)
//            {
//                firstFullCircles[i].visible = true;
//                // Use (firstModule.numBaseUnits-1) instead of math floor to catch when exactly 1 base unit (0 backing adjustments)
//                firstFullCircles[i].x = startingFirstCircle + (spanBetweenPieCenters * i);
//                firstFullCircles[i].y = newFirstPosition.y;
//            }
...
//            for (i in 0...firstSectors.length)
//            {
//                firstSectors[i].visible = true;
//                // If no partials, this should not run so edge case of exactly Math.floor( 1.0 ) or 2.0 should not occur.
//                firstSectors[i].x = newFirstPosition.x + Math.floor(first.fraction.value) * (spanBetweenPieCenters / 2);
//                firstSectors[i].y = newFirstPosition.y;
//            }
//
//            for (i in 0...secondSectors.length)
//            {
...
//            for (i in 0...firstFullCircles.length)
//            {
//                firstFullCircles[i].visible = false;
//            }
//            for (i in 0...secondFullCircles.length)
//            {
...
//            var denom_changeFirstSegmentDuration : Float = Math.min(m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DURATION_CHANGE_DENOM_PER_TICK) * ticksPerSegment,
//                    m_animHelper.computeScaledTime(PieConstants.TIME_ADD_DURATION_CHANGE_DENOM_TICKS_MAX)
//            );
